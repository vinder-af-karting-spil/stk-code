...
void ReplayRecorder::update(int ticks)
{
    ...
    for(unsigned int i=0; i<num_karts; i++)
    {
        ...
        if (attachment == -1)
        {
            Log::verbose("ReplayRecorder", "Unknown attachment type");
            return;
        }
        if (powerup_type == -1)
        {
            Log::verbose("ReplayRecorder", "Unknown powerup type");
            return;
        }
